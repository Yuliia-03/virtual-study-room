...
class LogInFormTestCase(TestCase):
    ...
    def setUp(self):
        self.form_input = {'email': 'alice@example.com', 'password': 'Password123'}
    ...
    def test_can_authenticate_valid_user(self):
        fixture = User.objects.get(email='bob@example.com')
        form_input = {'email': 'bob@example.com', 'password': 'Password123'}
        form = LogInForm(data=form_input)
        user = form.get_user()
        self.assertEqual(user, fixture)

    def test_invalid_credentials_do_not_authenticate(self):
        form_input = {'email': 'bob@example.com', 'password': 'WrongPassword123'}
        form = LogInForm(data=form_input)
        user = form.get_user()
        self.assertEqual(user, None)

    def test_blank_password_does_not_authenticate(self):
        form_input = {'email': 'bob@example.com', 'password': ''}
        form = LogInForm(data=form_input)
        user = form.get_user()
        self.assertEqual(user, None)
